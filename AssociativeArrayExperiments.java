...
public class AssociativeArrayExperiments {
  ...
  public static void expreimentStringsToStrings(PrintWriter pen) {
    ...
    s2s.set("A", "aardvark");
    s2s.size();
    AssociativeArray<String,String> s3s = s2s.clone();
    System.out.println(s3s + "original clone");
    s2s.hasKey("a");
    s2s.hasKey("A");
    try { s2s.get("a"); } catch (Exception e) { }
    try { s2s.get("A"); } catch (Exception e) { }
    s2s.remove("a");
    System.out.println(s3s + "clone change check");
    s2s.size();
    try { s2s.get("a"); } catch (Exception e) { }
    try { s2s.get("A"); } catch (Exception e) { }
    s2s.remove("aardvark");
  ...
  public static void experimentBigIntToBigInt(PrintWriter pen) {
    ...
    AssociativeArray<BigInteger,BigInteger> b3b = b2b.clone();
        System.out.println(b3b + "Clone Attempt");
    ...
    for (int i = 1; i < 11; i += 2) {
      b2b.remove(BigInteger.valueOf(i));
        ...
        System.out.println(b3b + "Clone remove check");
